...
# TODO(setup-wizard): rename this to your project's short identifier.
# It's used to pick the per-user state dir (~/.<PROJECT_NAME>/) and the
# default env-var prefix.
PROJECT_NAME: str = "setup-wizard"
...
# ---- TODO: add your repo-specific config-file paths here ----------------
#
# Example shape:
#
#     BOX_CONFIG_FILE: Path = Path(
#         os.environ.get("BOX_CONFIG_FILE", str(REPO_ROOT / "box-config.json"))
#     ).expanduser()
#     BOX_CONFIG_EXAMPLE: Path = REPO_ROOT / "box-config.json.example"
#
# Each entry should:
#   - accept an env-var override (so tests can substitute paths)
#   - default to a path under REPO_ROOT or Path.home()
#   - have a sibling .example file pointed to from the wizard's prompts
